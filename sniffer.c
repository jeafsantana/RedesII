...
void ProcessPacket(unsigned char* buffer, int size)
{
	...
	//inet_ntop(AF_INET6, &(iph->ip6_src.s6_addr), ipOrigem, INET6_ADDRSTRLEN);

    printf("ip: ");
    int i = 0;
    for(i = 0; i < 10; i++){
        printf("%x ", iph->ip6_dst.s6_addr[i]);
    }
    printf("\n");
...
void print_ip_header(unsigned char* Buffer, int Size)
{
	...
	// now get it back and print it
	inet_ntop(AF_INET6, &(iph->ip6_src), ipOrigem, INET6_ADDRSTRLEN);
